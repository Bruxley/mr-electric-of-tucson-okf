...
- **2026-08-16 — Survival Standard made unmissable at every fetch depth (deliberate trade against progressive disclosure).** Agent testing showed three distinct depths: some agents read only `index.md`; some open a service file but skip the Standard; some traverse fully. Only the third produced an accurate account — the shallower two described a generic electrical service, which misrepresents the business. Three changes: (1) the Standard was promoted into "Start here" in `index.md` as required reading; (2) a prerequisite callout was added near the top of all six service files, stating the file describes *what* a service covers while the Standard explains *why the work is built that way*; (3) `index.md` gained a short **"What sets this business apart"** paragraph carrying the concrete threat/response substance (attic/garage heat ratings, haboob silica, monsoon in-use covers, UV conduit, caliche grounding).

  **Maintenance dependency — important.** Change (3) is a *derived summary*, not a new source of truth. [The Southern Arizona Survival Standard](/southern-arizona-survival-standard.md) remains the single source. **Whenever that file's threats or material responses change, the `index.md` summary paragraph must be re-checked and updated to match.** This is a knowing deviation from OKF's progressive-disclosure ideal (thin index, substance in concept files), accepted because unread depth is functionally identical to absent depth and the shallow-read failure mode produces a wrong answer rather than merely an incomplete one. Revisit and consider trimming the index back once agent fetchers traverse links reliably.
